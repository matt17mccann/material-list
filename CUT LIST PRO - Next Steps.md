...
- **Unified PDF renderer** — All three output buttons (Save PDF / Share / Print) now hit the same dedicated render path (`FullReport`) instead of capturing whichever tab the user had active. The PDF currently includes Checklist on page 1 and Board Purchase Summary on its own final page. The per-unit Detailed Cut List still browses on-screen via the tab toggle but is intentionally held out of the PDF until the formatting is dialed in.
